...
public class ImageLoaderUtils {
	...
	// 返回注册号的imageloader对象
		...
		public static ImageLoader getInstance(Context context) {
			...
			File file = new File("/mnt/sdcard/DongDong/userhead/imageloader");
			// 制定sdcard缓存的路径
			confbuilder.discCache(new UnlimitedDiskCache(file));
			// 缓存的图片个数
		...
		public static DisplayImageOptions getOpt() {
			...
			// uri 加载默认图片
			//optbuilder.showImageForEmptyUri(R.drawable.icon_empty);
			// 图片获取失败 加载的默认图片
			//optbuilder.showImageOnFail(R.drawable.icon_error);
