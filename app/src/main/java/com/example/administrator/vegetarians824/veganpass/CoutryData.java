package com.example.administrator.vegetarians824.veganpass;


import com.example.administrator.vegetarians824.R;

/**
 * Created by Administrator on 2017-05-18.
 */
public class CoutryData {
    private  String[] CountryName = new String[]
            {       "阿尔巴尼亚","阿尔及利亚","阿根廷","阿拉伯联合酋长国","阿曼","阿塞拜疆","埃及","爱尔兰",
                    "爱沙尼亚","安哥拉","安提瓜和巴布达","奥地利","澳大利亚","巴巴多斯","巴布亚新几内亚","巴哈马","巴基斯坦",
                    "巴拉圭","巴勒斯坦","巴林","巴拿马","巴西","白俄罗斯","百慕大","保加利亚","北马里亚纳","贝宁",
                    "比利时","冰岛","波兰","波斯尼亚和黑塞哥维那","玻利维亚","伯利兹","博茨瓦纳","不丹","布基纳法索","布隆迪",
                    "朝鲜","赤道几内亚","丹麦","德国","东帝汶","多哥","多米尼加","俄罗斯","厄瓜多尔","厄立特里亚",
                    "法国","法属波利尼西亚","法属圭亚那","菲律宾","芬兰","佛得角","冈比亚","刚果布","刚果金",
                    "哥伦比亚","哥斯达黎加","格陵兰","格鲁吉亚","古巴","关岛","圭亚那","哈萨克斯坦","海地","韩国",
                    "荷属安的列斯","洪都拉斯","吉布提","吉尔吉斯斯坦","几内亚","几内亚比绍","加拿大","加纳",
                    "加蓬","柬埔寨","捷克","津巴布韦","喀麦隆","卡塔尔","开曼群岛","科摩罗","科索沃","科特迪瓦",
                    "科威特","克罗地亚","肯尼亚","库克群岛","拉脱维亚","莱索托","老挝","黎巴嫩","立陶宛","利比里亚",
                    "利比亚","列支敦士登","留尼旺","卢森堡","卢旺达","马达加斯加","马德拉群岛","马尔代夫","马耳他",
                    "马拉维","马来西亚","马里","马其顿","毛里求斯","毛里塔尼亚","美国","美属萨摩亚","蒙古","孟加拉国",
                    "秘鲁","缅甸","摩尔多瓦","摩洛哥","摩纳哥","莫桑比克","墨西哥","纳米比亚","南非","南苏丹",
                    "尼泊尔","尼加拉瓜","尼日尔","尼日利亚","纽埃","挪威","皮特凯恩岛","葡萄牙","日本","瑞典",
                    "瑞士","萨尔瓦多","塞尔维亚","塞拉利昂","塞内加尔","塞浦路斯","塞舌尔","沙特阿拉伯","圣多美和普林西比","圣赫勒拿",
                    "圣基茨和尼维斯","圣卢西亚","圣马力诺","圣文森特和格林纳丁斯","斯里兰卡","斯洛伐克","斯威士兰","苏丹",
                    "索马里","塔吉克斯坦","泰国","坦桑尼亚","特克斯和凯科斯群岛","特立尼达和多巴哥","突尼斯","土耳其","托克劳",
                    "瓦利斯与富图纳","危地马拉","委内瑞拉","文莱","乌干达","乌克兰","乌拉圭","乌兹别克斯坦","西班牙","西撒哈拉",
                    "希腊","新加坡","新喀里多尼亚","新西兰","匈牙利","叙利亚","牙买加","亚美尼亚","亚速尔群岛","也门",
                    "伊拉克","伊朗","以色列","意大利","印度","印度尼西亚","英国","约旦","越南","赞比亚",
                    "乍得","智利","中非","中国"};

    public String[] getCountryEnName() {
        return CountryEnName;
    }

    public void setCountryEnName(String[] countryEnName) {
        CountryEnName = countryEnName;
    }

    private String [] CountryEnName=new String[]{
            "Albania","Algeria","Argentina","United Arab Emirates","Oman","Azerbaijan","Egypt","Ireland","Estonia","Angola","Antigua and Barbuda","Austria","Australia","Barbados","Papua New Guinea","The Bahamas","Pakistan","Paraguay","Palestine","Bahrain","Panama","Brazil","Belarus","Bermuda","Bulgaria","Northern Mariana Islands","Benin","Belgium","Iceland","Poland","Bosnia and Herzegovina","Bolivia","Belize","Botswana","Bhutan","Burkina Faso","Burundi","North Korea","Equatorial Guinea","Denmark","Germany","East Timor","Togo","Dominican Republic","Russia","Ecuador","Eritrea","France","French Polynesia","French Guiana","Philippines","Finland","Cape Verde","Gambia","Republic of the Congo","Democratic Republic of the Congo","Colombia","Costa Rica","Greenland","Georgia","Cuba","Guahan","Guyana","Kazakhstan","Haiti","South Korea","Netherlands Antilles","Honduras","Djibouti","Kyrgyzstan","Guinea","Guinea Bissau","Canada","Ghana","Gabon","Cambodia","Czech Republic","Zimbabwe","Cameroon","Qatar","Cayman Islands","Comoros","Kosovo","Ivory Coast","Kuwait","Croatia","Kenya","The Cook Islands","Latvia","Lesotho","Laos","Lebanon","Lithuania","Liberia","Libya","Liechtenstein","Reunion Island","Luxembourg","Rwanda","Madagascar","Madeira Islands","Maldives","Malta","Malawi","Malaysia","Mali","Macedonia","Mauritius","Mauritania","United States of America","American Samoa","Mongolia","Bangladesh","Peru","Myanmar","Moldova","Morocco","Monaco","Mozambique","Mexico","Namibia","South Africa","South Sudan","Nepal","Nicaragua","Niger","Nigeria","Niue","Norway","Pitcairn Islands","Portugal","Japan","Sweden","Switzerland","El Salvador","Republic of Serbia","Sierra Leone","Senegal","Cyprus","Seychelles","Saudi Arabia","Sao Tome and Principe","Saint Helena","Saint Kitts and Nevis","Saint Lucia","San Marino","Saint Vincent and the Grenadines","Sri Lanka","Slovakia","Swaziland","Sudan","Somalia","Tajikistan","Thailand","United Republic of Tanzania","The Turks and Caicos Islands","Trinidad and Tobago","Tunisia","Turkey","Tokelau","Wallis et Futuna","Guatemala","Venezuela","Brunei","Uganda","Ukraine","Uruguay","Uzbekistan","Spain","Western Sahara","Greece","Singapore","New Caledonia","New Zealand","Austria","Syria","Jamaica","Armenia","Azores","Yemen","Iraq","Iran","Israel","Italy","India","Indonesia","United Kingdom","Jordan","Vietnam","Zambia","Chad","Chile","Central African Republic","China",
    };

    private int[][] CountryLanguage = new int[][]{
            //1-20
            {R.array.阿尔巴尼亚语,},
            {R.array.阿拉伯语,},
            //{R.array.普什图语,R.array.达里语,},
            {R.array.西班牙语,},
            {R.array.阿拉伯语,R.array.英语,},
            //{R.array.荷兰语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.阿塞拜疆语,R.array.俄语,},
            {R.array.阿拉伯语,},

            {R.array.英语,},
            {R.array.爱沙尼亚语,},
            //{R.array.加泰罗尼亚语,},
            {R.array.葡萄牙语,},
            {R.array.英语,},
            {R.array.德语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.英语,},

            // 21-40
            {R.array.乌尔都语,R.array.英语,},
            {R.array.西班牙语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.西班牙语,},
            {R.array.葡萄牙语,},
            {R.array.白俄罗斯语,R.array.俄语,},
            {R.array.英语,R.array.葡萄牙语,},
            {R.array.保加利亚语,},
            {R.array.英语},
            {R.array.法语,},
            {R.array.法语,R.array.德语,},
            {R.array.冰岛语,},
            {R.array.波兰语,},
            {R.array.波斯尼亚语,R.array.塞尔维亚语,R.array.克罗地亚语,},
            {R.array.西班牙语,},
            {R.array.英语,},
            {R.array.英语},
            {R.array.英语,},
            {R.array.法语,},

            //41=60
            {R.array.法语,},
            {R.array.韩语,},
            {R.array.西班牙语,R.array.法语,},
            {R.array.丹麦语,R.array.英语,R.array.德语,},
            {R.array.德语,},
            {R.array.葡萄牙语,R.array.英语,},
            {R.array.法语,},
            {R.array.西班牙语,},
            {R.array.俄语,},
            {R.array.西班牙语,},
            {R.array.阿拉伯语,},
            {R.array.法语,},
            // {R.array.法罗语,},
            {R.array.法语,},
            {R.array.法语},
            {R.array.英语,},
            {R.array.芬兰语,R.array.瑞典语,},
            {R.array.葡萄牙语,},
            {R.array.英语,},
            {R.array.法语,},

            //61-80
            {R.array.法语,},
            {R.array.西班牙语,},
            {R.array.西班牙语,},
            {R.array.丹麦语,},
            {R.array.格鲁吉亚语,},
            {R.array.西班牙语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.俄语,},
            {R.array.法语},
            {R.array.韩语,},
            //{R.array.荷兰语,},
            {R.array.西班牙语,R.array.英语,},
            //{R.array.黑山语,},
            {R.array.西班牙语,},
            {R.array.法语,R.array.阿拉伯语,},
            {R.array.俄语,},
            {R.array.法语,},
            {R.array.葡萄牙语},
            {R.array.英语,R.array.法语,},

            //81-100
            {R.array.英语,},
            {R.array.法语,},
            {R.array.高棉语,R.array.英语,},
            {R.array.捷克语,},
            {R.array.英语},
            {R.array.法语,R.array.英语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.英语,},
            {R.array.法语,R.array.阿拉伯语,},
            {R.array.阿尔巴尼亚语,R.array.塞尔维亚语,},
            {R.array.法语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.克罗地亚语,},
            {R.array.斯瓦希里语,R.array.英语,},
            {R.array.英语,},
            {R.array.俄语,},
            {R.array.英语},
            {R.array.老挝语,R.array.英语,},
            {R.array.阿拉伯语,R.array.英语,R.array.法语,},
            {R.array.白俄罗斯语,R.array.俄语,},

            //101-120
            {R.array.英语,},
            {R.array.阿拉伯语,},
            {R.array.德语,},
            {R.array.法语,},
            {R.array.卢森堡语,R.array.法语,R.array.德语,},
            {R.array.英语,R.array.法语,},
            //{R.array.罗马尼亚语,},
            {R.array.法语},
            {R.array.葡萄牙语,},
            {R.array.英语},
            {R.array.马耳他语,R.array.英语,},
            {R.array.英语},
            {R.array.马来语,R.array.英语,R.array.汉语,},
            {R.array.法语},
            {R.array.马其顿语,},
            {R.array.英语,R.array.法语,},
            {R.array.阿拉伯语,R.array.法语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.蒙古语,},

            //121-140
            {R.array.孟加拉语,R.array.英语,},
            {R.array.西班牙语,},
            {R.array.汉语,R.array.英语,},
            {R.array.俄语,},
            {R.array.阿拉伯语,R.array.法语,},
            {R.array.法语,},
            {R.array.葡萄牙语,},
            {R.array.西班牙语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.尼泊尔语,},
            {R.array.西班牙语,},
            {R.array.法语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.挪威语,},
            {R.array.英语,},
            {R.array.葡萄牙语,},
            {R.array.日语,},

            //141-160
            {R.array.瑞典语,},
            {R.array.德语,R.array.法语,R.array.意大利语},
            {R.array.西班牙语,},
            {R.array.塞尔维亚语,},
            {R.array.英语,},
            {R.array.法语,},
            {R.array.希腊语,R.array.土耳其语,R.array.英语,},
            {R.array.英语,R.array.法语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.葡萄牙语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.意大利语,},
            {R.array.英语,},
            {R.array.僧伽罗语,R.array.英语,},
            {R.array.斯洛伐克语,},
            //{R.array.斯洛文尼亚语,},
            {R.array.英语},
            {R.array.阿拉伯语,R.array.英语,},

            // 161-180
            //{R.array.荷兰语,},
            {R.array.索马里语,R.array.阿拉伯语,R.array.英语,R.array.意大利语,},
            {R.array.波斯语,},
            {R.array.泰语,R.array.英语,R.array.汉语,},
            {R.array.斯瓦希里语,R.array.英语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.阿拉伯语,},
            {R.array.土耳其语,R.array.阿塞拜疆语,},
            //{R.array.土库曼语,},
            {R.array.英语,},
            {R.array.法语},
            {R.array.西班牙语,},
            {R.array.西班牙语,},
            {R.array.马来语,R.array.英语,R.array.汉语,},
            {R.array.英语,R.array.斯瓦希里语,},
            {R.array.乌克兰语,R.array.俄语,},
            {R.array.西班牙语,},
            {R.array.乌兹别克语,R.array.俄语,},
            {R.array.西班牙语,},

            //181
            {R.array.阿拉伯语,R.array.西班牙语,R.array.法语,},
            {R.array.希腊语,R.array.英语,R.array.法语,},
            {R.array.英语,R.array.马来语,R.array.汉语},
            {R.array.法语},
            {R.array.英语,},
            {R.array.匈牙利语,},
            {R.array.阿拉伯语,R.array.英语,R.array.法语,},
            {R.array.英语,},
            {R.array.亚美尼亚语,},
            {R.array.葡萄牙语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.阿拉伯语,R.array.英语,R.array.波斯语,},
            {R.array.波斯语,},
            {R.array.希伯来语,R.array.阿拉伯语,R.array.英语,},
            {R.array.意大利语,R.array.英语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.英语,},
            {R.array.阿拉伯语,R.array.英语,},
            {R.array.越南语,},
            {R.array.英语,},
            {R.array.法语,R.array.法语,},
            {R.array.西班牙语,},
            {R.array.法语},
            {R.array.汉语,}
    };



    private  String[][] CountryNameA2Z= new String[][]
            {       //A-Z排序
                    //a
                    {"阿尔巴尼亚","阿尔及利亚","阿根廷","阿拉伯联合酋长国","阿曼","阿塞拜疆","埃及","爱尔兰",
                            "爱沙尼亚","安哥拉","安提瓜和巴布达","奥地利","澳大利亚"},
                    //b
                    {"巴巴多斯","巴布亚新几内亚","巴哈马","巴基斯坦",
                            "巴拉圭","巴勒斯坦","巴林","巴拿马","巴西","白俄罗斯","百慕大","保加利亚","北马里亚纳","贝宁",
                            "比利时","冰岛","波兰","波斯尼亚和黑塞哥维那","玻利维亚","伯利兹","博茨瓦纳","不丹","布基纳法索","布隆迪"},
                    //c
                    {"朝鲜","赤道几内亚"},
                    //d
                    {"丹麦","德国","东帝汶","多哥","多米尼加"},
                    //e
                    {"俄罗斯","厄瓜多尔","厄立特里亚"},
                    //f
                    {"法国","法属波利尼西亚","法属圭亚那","菲律宾","芬兰","佛得角"},
                    //g
                    {"冈比亚","刚果布","刚果金", "哥伦比亚","哥斯达黎加","格陵兰","格鲁吉亚","古巴","关岛","圭亚那"},
                    //h
                    {"哈萨克斯坦","海地","韩国","荷属安的列斯","洪都拉斯"},
                    //i

                    //j
                    {"吉布提","吉尔吉斯斯坦","几内亚","几内亚比绍","加拿大","加纳", "加蓬","柬埔寨","捷克","津巴布韦"},
                    //k
                    {"喀麦隆","卡塔尔","开曼群岛","科摩罗","科索沃","科特迪瓦", "科威特","克罗地亚","肯尼亚","库克群岛"},
                    //l
                    {"拉脱维亚","莱索托","老挝","黎巴嫩","立陶宛","利比里亚",
                            "利比亚","列支敦士登","留尼旺","卢森堡","卢旺达"},
                    //m
                    {"马达加斯加","马德拉群岛","马尔代夫","马耳他",
                            "马拉维","马来西亚","马里","马其顿","毛里求斯","毛里塔尼亚","美国","美属萨摩亚","蒙古","孟加拉国",
                            "秘鲁","缅甸","摩尔多瓦","摩洛哥","摩纳哥","莫桑比克","墨西哥"},
                    //n
                    {"纳米比亚","南非","南苏丹",
                            "尼泊尔","尼加拉瓜","尼日尔","尼日利亚","纽埃","挪威"},
                    //o

                    //p
                    {"皮特凯恩岛","葡萄牙"},
                    //q

                    //r
                    {"日本","瑞典", "瑞士"},
                    //s
                    {"萨尔瓦多","塞尔维亚","塞拉利昂","塞内加尔","塞浦路斯","塞舌尔","沙特阿拉伯","圣多美和普林西比","圣赫勒拿",
                            "圣基茨和尼维斯","圣卢西亚","圣马力诺","圣文森特和格林纳丁斯","斯里兰卡","斯洛伐克","斯威士兰","苏丹",
                            "索马里"},
                    //t
                    {"塔吉克斯坦","泰国","坦桑尼亚","特克斯和凯科斯群岛","特立尼达和多巴哥","突尼斯","土耳其","托克劳"},
                    //u

                    //v
                    {"瓦利斯与富图纳","危地马拉","委内瑞拉","文莱","乌干达","乌克兰","乌拉圭","乌兹别克斯坦"},
                    //w

                    //x
                    {"西班牙","西撒哈拉", "希腊","新加坡","新喀里多尼亚","新西兰","匈牙利","叙利亚"},
                    //y
                    {"牙买加","亚美尼亚","亚速尔群岛","也门", "伊拉克","伊朗","以色列","意大利","印度","印度尼西亚","英国","约旦","越南"},
                    //z
                    {"赞比亚", "乍得","智利","中非","中国"}

            };

    public CoutryData(){

    }



    public String[][] getCountryNameA2Z() {
        return CountryNameA2Z;
    }

    public void setCountryNameA2Z(String[][] countryNameA2Z) {
        CountryNameA2Z = countryNameA2Z;
    }

    public String[] getCountryName() {
        return CountryName;
    }

    public void setCountryName(String[] countryName) {
        CountryName = countryName;
    }



    public int[][] getCountryLanguage() {
        return CountryLanguage;
    }

    public void setCountryLanguage(int[][] countryLanguage) {
        CountryLanguage = countryLanguage;
    }


}
