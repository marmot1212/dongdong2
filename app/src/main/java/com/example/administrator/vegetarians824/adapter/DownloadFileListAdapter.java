...
import com.example.administrator.vegetarians824.util.ApkUtil;
import com.example.administrator.vegetarians824.util.DipPX;
import com.example.administrator.vegetarians824.util.TimeUtil;
import com.example.administrator.vegetarians824.util.VideoHelper;
...
public class DownloadFileListAdapter extends BaseAdapter implements OnRetryableFileDownloadStatusListener {
    ...
    private int Status_Select;

    public DownloadFileListAdapter(BaseActivity activity,int status) {
        super();
    ...
    private void initDownloadFileInfos() {
        ...
        List<DownloadFileInfo> filelist=new ArrayList<>();
        List<DownloadFileInfo> mytask=FileDownloader.getDownloadFiles();
        for(int i=0;i<mytask.size();i++){
            if(Status_Select==0){
                if(mytask.get(i).getStatus()==Status.DOWNLOAD_STATUS_COMPLETED){
                    filelist.add(mytask.get(i));
                }
            }else {
                if(Status_Select==1){
                    if(mytask.get(i).getStatus()==Status.DOWNLOAD_STATUS_PAUSED||mytask.get(i).getStatus()==Status.DOWNLOAD_STATUS_DOWNLOADING){
                        filelist.add(mytask.get(i));
                    }
                }
            }
        ...
        this.mDownloadFileInfos = filelist;
        mConvertViews.clear();
        ...
        */
        ...
        switch (downloadFileInfo.getStatus()) {
            ...
            case Status.DOWNLOAD_STATUS_COMPLETED:
                ...
                params.height= DipPX.dip2px(mActivity.getBaseContext(),60);
                params.width=DipPX.dip2px(mActivity.getBaseContext(),80);
                ivIcon.setLayoutParams(params);
                ivIcon.requestLayout();
                ivIcon.setImageBitmap(VideoHelper.getVideoThumbnail(downloadFileInfo.getFilePath()));
                ...
                try {
                    ...
                    bitmap = retriever.getFrameAtTime(3, MediaMetadataRetriever.OPTION_CLOSEST_SYNC);
                        ...
                        case Status.DOWNLOAD_STATUS_COMPLETED:
                            ...
                            */
                            ...
                            if(true){
                                ...
                                Uri data = Uri.parse(curDownloadFileInfo.getFilePath());
                                intent.setDataAndType(data, "video/mp4");
                                mActivity.startActivity(intent);
                                */
                                Intent intent=new Intent(mActivity, VideoDetail.class);
                                intent.putExtra("url",curDownloadFileInfo.getUrl());
                                mActivity.startActivity(intent);
                            }
                            break;
                    }
